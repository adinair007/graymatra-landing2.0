...
<template>
  ...
  <div class="hidden md:flex h-96 w-full gap-4" :class="[props.class]">
    <div
  ...
  <div class="md:hidden w-full space-y-4">
    <div
      v-for="(image, index) in images"
      :key="`mobile-${image}`"
      class="group relative w-full overflow-hidden rounded-3xl border border-white/10 hover:border-[#ff9933]/50 transition-all duration-500"
    >
      ...
      <div
        v-if="titles && titles[index]"
        class="relative bg-gradient-to-t from-black/95 via-black/80 to-transparent p-6"
      >
        <!-- Glow effect -->
        <div class="absolute inset-0 pointer-events-none">
          <div
            class="absolute inset-0 bg-[radial-gradient(circle_at_50%_80%,rgba(255,153,51,0.1),transparent_70%)]"
          ></div>
        </div>
